...
PbMetalensDesign design_pb_metalens(const HwpAtom& atom, double period_um,
    ...
    const double lambda = wavelength_um;
    ...
    // Hyperbolic focusing phase; geometric phase imprinted on the cross output
    // is -handedness * 2*theta, so theta = -handedness * phi / 2 hits phi.
    auto target_phase = [&](double r) {
        return -(2.0 * pi / lambda) * (std::sqrt(r * r + focal_length_um * focal_length_um) -
                                       focal_length_um);
    };
    // The spin-flip amplitude of the un-rotated atom (uniform across the lens).
    // Its absolute phase is a global piston, irrelevant to focusing -- the only
    // thing that matters is that the per-site phase tracks phi(r).
